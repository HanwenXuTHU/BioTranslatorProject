...
def main():
    ...
    nocl_path = "/disk1/hwxu/protein2def/proteinTextomics/results/metrics/goa_human_eval_nocl_fold_{}.pkl"
    n_fold = 5
    textomics_flat = collections.OrderedDict()
    nocl_flat = collections.OrderedDict()
    ...
    for i in range(n_fold):
        ...
        nocl = load_obj(nocl_path.format(i))
        for j in range(0, 5):
        ...
        for j in range(0, 5):
            nocl_flat[metrics[j]].append(nocl['bleu']['{}_avg'.format(metrics[j])])
        for j in range(5, 8):
            nocl_flat[metrics[j]].append(nocl['rouge']['{}_avg'.format(metrics[j])])
        nocl_flat['meteor'].append(nocl['meteor'])

    mean = np.zeros([len(metrics), 2])
    yerr = np.zeros([len(metrics), 2])
    i = 0
    for m in metrics:
        mean[i, 0], mean[i, 1] = np.mean(textomics_flat[m]), np.mean(nocl_flat[m])
        yerr[i, 0], yerr[i, 1] = np.std(textomics_flat[m]), np.std(nocl_flat[m])
        i += 1
    ...
    ax.bar(index + (nmethod - 1 - 1) * bar_width, mean[:, 0], yerr=yerr[:, 0], width=bar_width, alpha=opacity,
           ...
           label='With Contrastive Learning')
    ...
    #plt.show()
    plt.savefig('pdfs/compare_cl.pdf')
