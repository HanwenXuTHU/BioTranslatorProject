...
def auroc_bar_percentage_plot(percentage_BioTranslator, save_path):
    ...
    Borrowed from OnClass paper
    '''
    results = percentage_BioTranslator
    T = [0.65, 0.7, 0.75, 0.8, 0.85, 0.9, 0.95]
    percentage_i = list(results.values())
    mean = np.zeros([len(percentage_i), 1])
    mean[:, 0] = percentage_i
    ...
    ax.bar(index + (nmethod - 1 - 0) * bar_width, mean[:, 0], width=bar_width, alpha=opacity,
           color='#66c2a5')  # ,color_l[i],

    csfont = {'family': 'Helvetica'}
    ax.set_ylabel('Percentage of functions \n AUROC > threshold', fontdict=csfont)
    ax.set_xticklabels(['0.65', '0.70', '0.75', '0.80', '0.85', '0.90', '0.95'], fontsize=5)
    ...
    if nmethod == 1:
        ...
        ax.set_xticks(index + bar_width * (nmethod - 0.5) * 1. / 2 - 0.1)
    # plt.legend(bbox_to_anchor=(1.5, 1), frameon=False, ncol=1, fontsize=4)
