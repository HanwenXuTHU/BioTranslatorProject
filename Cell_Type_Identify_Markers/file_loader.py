...
class cellData(Dataset):
    ...
    def __init__(self, expression_matrix, labels, nlabel=0, is_gpu=True, memory_saving_mode=True):
    ...
    def __getitem__(self, item):
        if self.memory_saving_mode:
            ...
            x = np.asarray(self.expression_matrix[item, :]).squeeze()
...
class FileLoader:
    ...
    def generate_data(self, iter, unseen_ratio):
        ...
        if self.opt.memory_saving_mode:
            ...
            test_feature = self.DataProcess_obj.ProcessTestFeature(test_feature, test_genes, log_transform=False)


        self.unseen2i = collections.OrderedDict()
        self.train2i = collections.OrderedDict()
        self.test2i = collections.OrderedDict()
        self.i2test = collections.OrderedDict()
        self.i2unseen = collections.OrderedDict()
        idx = 0
        ...
        self.train_data = cellData(train_feature, train_label,
                                   nlabel=len(self.train2i.keys()),
                                   is_gpu=self.opt.is_gpu,
                                   memory_saving_mode=self.opt.memory_saving_mode)
        self.test_data = cellData(test_feature, test_label,
                                  nlabel=len(self.test2i.keys()),
                                  is_gpu=self.opt.is_gpu,
                                  memory_saving_mode=self.opt.memory_saving_mode)
